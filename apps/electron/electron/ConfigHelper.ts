...
export class ConfigHelper extends EventEmitter {
  ...
  private defaultConfig: Config = {
    ...
    opacity: 1.0,
    memoryEnabled: true,
    // Postgres.app default: peer auth on localhost, no password, current macOS user.
    // Override with env MEMORY_DB_URL or via Settings UI.
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic" | "xai" | "groq"): string {
    if (provider === "openai") {
      ...
      const allowedModels = ["grok-beta", "grok-vision-beta"];
      return allowedModels.includes(model) ? model : "grok-beta";
    } else if (provider === "groq") {
      // Allow the specific model requested by user, plus other common Groq models
      const allowedModels = ["meta-llama/llama-guard-4-12b", "openai/gpt-oss-120b", "meta-llama/llama-4-scout-17b-16e-instruct", "meta-llama/llama-4-maverick-17b-128e-instruct", "llama-3.3-70b-versatile", "llama3-70b-8192", "llama3-8b-8192", "mixtral-8x7b-32768", "gemma-7b-it"];
      if (!allowedModels.includes(model)) {
        // If the model is not in our strict list, we might still want to allow it if it looks valid, 
        // but for safety let's default to the one the user specifically asked for if invalid.
  ...
  public loadConfig(): Config {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        // text-only model, which crashes the debug flow because it sends
        // image screenshots. Force back to scout (the only Groq vision model).
        if (config.apiProvider === "groq" && config.debuggingModel === "llama-3.3-70b-versatile") {
          config.debuggingModel = "meta-llama/llama-4-scout-17b-16e-instruct";
        }
    ...
    try {
      ...
      if (updates.apiKey && !updates.apiProvider) {
        if (updates.apiKey.trim().startsWith('sk-')) {
          ...
          provider = "xai";
          console.log("Auto-detected xAI API key format");
      ...
      if (updates.apiProvider && updates.apiProvider !== currentConfig.apiProvider) {
        ...
        } else if (updates.apiProvider === "xai") {
          updates.extractionModel = "grok-vision-beta";
          updates.solutionModel = "grok-beta";
          updates.debuggingModel = "grok-beta";
        } else if (updates.apiProvider === "groq") {
          // Extraction + debug both pass screenshots, so they need scout
          // (the only Groq vision model). Solution is pure text reasoning,
          // so use 3.3-70b for the strongest coding output on the free tier.
      ...
      if (updates.apiKey !== undefined || updates.apiProvider !== undefined ||
          ...
          updates.memoryEnabled !== undefined || updates.memoryConnectionString !== undefined ||
          updates.yenModeEnabled !== undefined) {
   ...
   */
  public async testApiKey(apiKey: string, provider?: "openai" | "gemini" | "anthropic" | "xai" | "groq"): Promise<{valid: boolean, error?: string}> {
    ...
    if (!provider) {
      if (apiKey.trim().startsWith('sk-')) {
        ...
        provider = "xai";
        console.log("Auto-detected xAI API key format for testing");
      } else if (apiKey.trim().startsWith('gsk_')) {
        provider = "groq";
    ...
    if (provider === "openai") {
      ...
      try {
        ...
        return { valid: false, error: error.message || 'Unknown error validating xAI API key' };
      }
    } else if (provider === "groq") {
  ...
  private async testGroqKey(apiKey: string): Promise<{valid: boolean, error?: string}> {
    try {
      // Use axios to test the key against Groq API
