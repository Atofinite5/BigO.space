...
const electronAPI = {
  ...
  updateConfig: (config: { apiKey?: string; model?: string; language?: string; opacity?: number; memoryEnabled?: boolean; memoryConnectionString?: string; yenModeEnabled?: boolean }) =>
    ipcRenderer.invoke("update-config", config),

  // Yen (in-memory bucket mode) controls
  yenGetState: () => ipcRenderer.invoke("yen-get-state"),
  yenTogglePanel: () => ipcRenderer.invoke("yen-toggle-panel"),
  yenClosePanel: () => ipcRenderer.invoke("yen-close-panel"),
  yenOpenBucket: (bucketIndex: number) =>
    ipcRenderer.invoke("yen-open-bucket", bucketIndex),
  yenProcessCaptures: () => ipcRenderer.invoke("yen-process-captures"),
  yenDeleteBucket: (bucketIndex: number) =>
    ipcRenderer.invoke("yen-delete-bucket", bucketIndex),
  yenClearAll: () => ipcRenderer.invoke("yen-clear-all"),
  yenClearCaptures: () => ipcRenderer.invoke("yen-clear-captures"),
  yenCopyText: (text: string) => ipcRenderer.invoke("yen-copy-text", text),
  onYenState: (callback: (state: any) => void) => {
    const sub = (_: any, s: any) => callback(s)
    ipcRenderer.on("yen-state", sub)
    return () => ipcRenderer.removeListener("yen-state", sub)
  },
  onYenProcessResult: (
    callback: (result: { ok: boolean; error?: string; bucketIndex?: number }) => void
  ) => {
    const sub = (_: any, r: any) => callback(r)
    ipcRenderer.on("yen-process-result", sub)
    return () => ipcRenderer.removeListener("yen-process-result", sub)
  },
  onYenDisabledHint: (callback: () => void) => {
    const sub = () => callback()
  ...
  testMemoryConnection: (connectionString?: string) =>
    ipcRenderer.invoke("memory-test-connection", connectionString || ""),
  reinitMemory: () => ipcRenderer.invoke("memory-reinit"),
  setupMemoryDatabase: (connectionString?: string) =>
    ipcRenderer.invoke("memory-setup-database", connectionString || ""),
  likeMemory: (id: number, liked: boolean) =>
    ipcRenderer.invoke("memory-like", id, liked),
  // Click-through: renderer tells main when the mouse is over a UI element.
  ...
  onToggleTheme: (callback: () => void) => {
    const sub = () => callback()
    ipcRenderer.on("toggle-theme", sub)
    return () => ipcRenderer.removeListener("toggle-theme", sub)
  },
  ...
  onWindowReshown: (callback: () => void) => {
    const subscription = () => callback()
    ipcRenderer.on("window-reshown", subscription)
    return () => {
      ipcRenderer.removeListener("window-reshown", subscription)
    }
  },
