...
const SubscribedApp: React.FC<SubscribedAppProps> = ({
  ...
  const [yenPanelOpen, setYenPanelOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    window.electronAPI.yenGetState?.().then((s: { panelOpen: boolean } | null) => {
      if (alive && s) setYenPanelOpen(s.panelOpen)
    })
    const off = window.electronAPI.onYenState?.((s: { panelOpen: boolean }) =>
      setYenPanelOpen(s.panelOpen)
    )
    return () => {
      alive = false
      off?.()
