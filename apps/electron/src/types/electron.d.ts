export interface ElectronAPI {
  ...
  yenTogglePanel: () => Promise<any>
  yenClosePanel: () => Promise<any>
  yenOpenBucket: (bucketIndex: number) => Promise<any>
  yenProcessCaptures: () => Promise<{ ok: boolean; error?: string; bucketIndex?: number }>
  yenDeleteBucket: (bucketIndex: number) => Promise<{ ok: boolean }>
  yenClearAll: () => Promise<{ ok: boolean }>
  yenClearCaptures: () => Promise<{ ok: boolean }>
  yenCopyText: (text: string) => Promise<{ ok: boolean; error?: string }>
  onYenState: (callback: (state: any) => void) => () => void
  onYenProcessResult: (
    callback: (result: { ok: boolean; error?: string; bucketIndex?: number }) => void
  ) => () => void
  onYenDisabledHint: (callback: () => void) => () => void

  likeMemory: (id: number, liked: boolean) => Promise<{ ok: boolean }>
